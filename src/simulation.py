...
from simulation_environment.simulation_engine import Simulation
from waitress import serve

config_path, base_url, port, api_port, secret = sys.argv[1:]
...
def register_agent():
    ...
    for event in events['current_event']:
        if event == 'flood' and events['current_event'][event]:
            events['current_event'][event] = events['current_event'][event].json()
        else:
            events['current_event'][event] = [event.json() for event in events['current_event'][event]]
